...
def test_memory_paths_are_lazy_until_memory_is_written(tmp_path: Path) -> None:
...
def test_schema_migration_and_validation_guards(tmp_path: Path) -> None:
    ...
    with pytest.raises(MemorySchemaError, match="Unsupported"):
...
def test_old_versioned_schema_is_not_migrated(tmp_path: Path) -> None:
    paths = ClientPaths(tmp_path / "old")
