...
def test_phase2_cancellation_before_commit_leaves_candidates_pending(tmp_path: Path) -> None:
    store = MemoryStore(ClientPaths(tmp_path / "user"))
    record = _episodic("session_cancel", "cancel")
    _record(store, record)

    with pytest.raises(RuntimeError, match="cancelled"):
        ManualMemoryConsolidator(store, _AddingModel(add_procedural=False), clock=lambda: T1).consolidate(
            before_commit=lambda: (_ for _ in ()).throw(RuntimeError("cancelled"))
        )

    assert store.get_candidate(record.candidate.candidate_id).status is MemoryCandidateStatus.PENDING  # type: ignore[union-attr]
    assert store.list_items(kinds=(MemoryKind.SEMANTIC, MemoryKind.PROCEDURAL)) == []
