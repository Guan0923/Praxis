...
每个 Thread 同时最多一个活动 Turn。稳定聊天历史写入 SQLite；浏览器待发送草稿与 active Turn steering 进入 进程内队列，使用 `delivery_id` 持久幂等。SSE 断线只取消订阅，不取消后台 worker。Job registry 按 system/session/thread/run/task 组织，不含账户 owner。
...
Provider wire formats只存在于 `backend/src/providers/`。Runtime 发布事件，UI 展示逻辑留在前端；`delivery_id` 只进入 SQLite/Node Message 元数据，不进入 Provider 请求。SQLite 仍是正式历史权威，内存队列 不承担历史投影。
