...
- `accounts.py`：`provision_fixed_accounts` 创建或验证 `PraxisSbxOffline`/`PraxisSbxOnline` 与受管本地组，轮换并 DPAPI 持久化凭据，授予最小登录权；`remove_owned_accounts` 只删除可由凭据包和 SID 证明归属的账户；其余 helper 负责凭据探测和冲突检查。
