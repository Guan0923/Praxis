...
class MemoryStore:
    ...
    def record_phase1_batch(
        ...
        *,
        ...
        with self._connection(create=True, immediate=True) as connection:
            assert connection is not None
            self._require_running_job(connection, job_id, lease_owner)
            current = self._watermark_row(connection, watermark.source_id)
            if current is not None and int(current["position"]) > watermark.position:
                raise MemoryConflictError("Memory watermark cannot move backwards.")
            try:
    ...
    def _require_running_job(
        ...
        if job_id is None:
            return
        row = connection.execute(
            "SELECT status,lease_owner FROM memory_jobs WHERE job_id=?",
            (job_id,),
        ).fetchone()
        if row is None or row["status"] != MemoryJobStatus.RUNNING.value or row["lease_owner"] != lease_owner:
            raise MemoryConflictError("Memory job was cancelled before its write transaction.")
    ...
    def _validate_existing_paths(self) -> None:
        ...
        if self.paths.raw_memories_file.is_symlink():
            raise MemoryStorageError("Memory projection cannot be a symbolic link.")
        if self.paths.rollout_summaries_dir.is_symlink():
            raise MemoryStorageError("Memory rollout directory cannot be a symbolic link.")
        ...
        if self.paths.raw_memories_file.exists() and not self.paths.raw_memories_file.is_file():
            raise MemoryStorageError("Memory projection path must be a regular file.")
        if self.paths.rollout_summaries_dir.exists() and not self.paths.rollout_summaries_dir.is_dir():
            raise MemoryStorageError("Memory rollout path must be a directory.")
